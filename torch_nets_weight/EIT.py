...
from attackIT.attack_methods import DI, gkern
from Normalize import Normalize, Re_normalize
from utils import ShowGrayscaleImage, ShowColorscaleImage, save_saliency_map,overlay_mask
...
class EIT:
    ...
    def vis(self, x, t):
        ...
        # x = Re_normalize(x, mean = [0.5,0.5,0.5], std = [0.5,0.5,0.5])
        x = x.detach().cpu().squeeze().numpy().transpose(1, 2, 0)
        plt.imshow(x)
        plt.savefig(f"{t}.png", dpi=750)
        plt.show()
        
    def show_as_gray_image(self, img,percentile=99):
        import os
        os.environ['KMP_DUPLICATE_LIB_OK'] = 'True'
        import matplotlib.pyplot as plt
        import cv2 
    ...
    def __call__(self, model: nn.Module, inputs: torch.tensor, labels: torch.tensor, **kwargs):
        ...
        for i in range(self.steps):
            ...
            ShowGrayscaleImage(grad * grad, "b2")
